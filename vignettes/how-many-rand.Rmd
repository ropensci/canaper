...
# Plot the results
ggplot(dist_test_res, aes(x = n_iter, y = dist)) +
  geom_point() +
  geom_line() +
  scale_x_log10() +
  labs(x = "Num. iterations", y = "Distance")
```

From this, we can see that the original community and the randomized community reach a maximum distance at ca. 1,000 to 10,000 iterations. After that, the distances level off, and don't increase with additional "mixing"^[In fact with too much mixing, we might start to come back to the original!].
...
In this case (the [example dataset](https://docs.ropensci.org/canaper/reference/biod_example.html) that comes with `canaper`), we see that a minimum of 1,000 random replicates with 10,000 swapping iterations per replicate is probably needed to attain robust results. 
