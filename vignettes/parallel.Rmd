...
biod_res_seq <- cpr_rand_test(
  ...
  null_model = "independentswap", n_reps = 50)
toc()
```
...
To enable parallel computing, just add one line before `cpr_rand_test()`: `plan(multisession, workers = 4)` ^[Of course, the number of workers should be no greater than the number of available CPUs.]. Here, the `multisession, workers = 3` part is telling `future` that we want to use 3 CPUs in parallel on our local machine. See `future::plan()` for other options. Otherwise, **everything is the same**.

```{r parallel-1}
# Set up parallel computing with 3 CPUs
...
This time, the calculations were carried out in 3 batches in parallel.

But there is no significant improvement in runtime^[Results vary each time this vignette is generated, but it probably won't be three times as fast; sometimes, the parallel version is even **slower**.]. What is going on here?
...
```{r acacia-parallel}
# Run cpr_rand_test() in parallel with 3 CPUs
plan(multisession, workers = 3)
tic()
...
If you'd like to track the progress of `cpr_rand_test()` in real time, you can enable a progress bar. There are two ways to do so (neither of these will show up on the webpage, but do try it at home!).
...
biod_res_long <- cpr_rand_test(
  biod_example$comm, biod_example$phy, 
  null_model = "independentswap", n_reps = 500)
```
