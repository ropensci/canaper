...
This vignette replicates the [analysis of @Mishler2014](https://doi.org/10.1038/ncomms5473), where CANAPE was originally defined. 
...
acacia_rand_res <- cpr_rand_test(acacia$comm, acacia$phy, n_reps = 100, n_iterations = 100000)
toc() # See how long it took
```

```{r acacia-rand-temp}
acacia_rand_res <- readRDS(here::here("data-raw/acacia_rand_res.RDS"))
```

If we hadn't run it in parallel, it would have taken 4 times longer!
...
```{r acacia-rand-res}
tibble::as_tibble(acacia_rand_res, rownames = "site")
```
