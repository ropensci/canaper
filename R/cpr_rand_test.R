...
cpr_rand_test <- function(comm, phy, null_model,
  ...
  # Add warning for abundance data
  if (isTRUE(any(unlist(comm) > 1)) && !quiet) {
    warning(
      "Abundance data detected. Results will be the same as if using presence/absence data (no abundance weighting is used).", # nolint
      call. = FALSE,
      immediate. = TRUE
    )
  }
  ...
  if (
    percent_present < 0.05 &&
      n_reps_input == 100 &&
      n_iterations_input == 10000 && !quiet
  ) {
    # nolint
    warning(
