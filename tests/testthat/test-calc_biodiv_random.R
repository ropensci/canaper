...
test_that("Input is valid", {
	...
	expect_error(
		calc_biodiv_random(comm, NA, phy_alt, "independentswap", 10L),
		"'phy' must be a list of class 'phylo'"
	)
	expect_error(
		calc_biodiv_random(comm, phy, phy_alt, "independentswap", 10L, metrics = "pg"),
		"'metrics' may only include 'pd', 'rpd', 'pe', 'rpe', 'pd_alt', 'pe_alt'"
	)
	expect_error(
		calc_biodiv_random(comm, phy, phy_alt, "indepswap", 10L, metrics = "pd"),
		"'null_model' must be one of 'frequency', 'richness', 'independentswap', or 'trialswap'"
	)
	expect_error(
		calc_biodiv_random(comm[1:3,1:3], phy, phy_alt, "independentswap", 10L),
	...
	expect_error(
		calc_biodiv_random(comm, phy, phy, "independentswap", 10L),
...
test_that("Selected metric shows up in output", {
	expect_named(calc_biodiv_random(comm, phy, phy_alt, "independentswap", 10L, metrics = "pd"), "pd")
	expect_named(calc_biodiv_random(comm, phy, phy_alt, "independentswap", 10L, metrics = "pe"), "pe")
	expect_named(calc_biodiv_random(comm, phy, phy_alt, "independentswap", 10L, metrics = "rpd"), c("pd", "pd_alt", "rpd"), ignore.order = TRUE)
	expect_named(calc_biodiv_random(comm, phy, phy_alt, "independentswap", 10L, metrics = "rpe"), c("pe", "pe_alt", "rpe"), ignore.order = TRUE)
	expect_named(
		calc_biodiv_random(comm, phy, phy_alt, "independentswap", 10L, metrics = c("rpd", "rpe")),
		c("pd", "pd_alt", "rpd", "pe", "pe_alt", "rpe"), ignore.order = TRUE
	)
	expect_named(
		calc_biodiv_random(comm, phy, phy_alt, "independentswap", 10L, metrics = c("pd", "pe")),
		c("pd", "pe"),
		ignore.order = TRUE
	)
	expect_named(
		calc_biodiv_random(comm, phy, phy_alt, "independentswap", 10L, metrics = c("pd", "pe", "pd_alt")),
		c("pd", "pe", "pd_alt"),
		ignore.order = TRUE
	)
})
